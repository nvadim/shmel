    ...
    private function preparePage()
    {
        ...
        if ($step != 'route') {
            ...
            for ($i = 0; $i < count($this->_savedData['route']['FROM']); $i++) {
                ...
                $this->arResult['select_route']["reference_id"][] = "from_{$i}";
            }
            $this->arResult['select_route']["reference"][] = $this->_savedData['route']['TO'];
    ...
    public function selectKit()
    {
        ...
        for ($i = 0; $i < count($KITS); $i++) {
            ...
            if ($strData->NumberOfRooms == $numRooms
                ...
                $this->saveTransport2Loc($this->_sessData, $KITS[$i]->StructTransports);
